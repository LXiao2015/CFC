double cost(int i, struct CFC Chains[], int ins) {
	double CR = 0.0, CF = 0.0, CFF = 0.0, CU = 0.0;
	
	for(int step = 0; Chains[i].update[ins].upath[step] > 0; ++step) {
		CR += use_cost_of_node[Chains[i].update[ins].upath[step] - 1] * Chains[i].demand;
	}
...
//	cout<<endl;
	CU += (1 - (int)(Chains[i].update[ins].unode == Chains[i].ini_node)) * update_msg_cost;    // 这个是瞎写的, 应该与最开始分配的结果比较 
